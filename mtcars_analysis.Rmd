---
title: "Motor Trends Dataset MPG Analysis"
...
- We look at mtcars data, 32 observations of 11 variables, collected for 1973-74 models
- Specifically we want to understand and quantify the impact on MPG by automatic versus manual transmission vehicles
- It's not straightforward, there are some confounders, so we have to pit some models against each other and eliminate them until we find the best one.
- Once found, we'll quantify the impact on MPG by Manual and Auto transmission vehicles, and see if this difference is significant. 

# Building Models

Since we have the one response variable that we're trying to predict for - "MPG" - we have 10 remaining variables. All of these could be considered as potential predictors of MPG. We attempt to pursue different strategies to pick the most promising potential predictors and therefore, as a first step, build competing models.

## MODEL 1 (Linear Correlation): Most Correlated Response-Predictor Pairs

For the first model, we look at scatter plots of the response - MPG - against every potential predictor in Figure 1.1. We filter predictors based on the following criteria:  
- Adjusted R^2
- Slope coefficient that's significant. 
- Approximate linear relationship with fitted line, few outliers accepted.

We thus arrive at the model (including "am" by default), in decreasing strength of R2:

mpg ~ wt + cyl + disp + hp + am
```{r}
model1 <- lm(mpg~wt+cyl+disp+hp+am, data=mtcars)
```


## MODEL 2 (STEPWISE ADDITION/ELIMINATION): Arriving at Ideal AIC Score

The stepwise function will be called on the _mpg~._ model to iterate through _Forward Addition_ and _Backwards Elimination_ procedures and pick the models with the most optimal AIC value:  

```{r}
model2.step <- step(lm(mpg~., data=mtcars), direction="both", trace=FALSE)
model2.step$coefficients
model2 <- lm(mpg~wt+qsec+am, data=mtcars)
```  

We see that the most optimal model coming out of Stepwise Addition/Elimination is 

mpg ~ wt + qsec + am.


## MODEL 3 (Correlation and VIF Analysis)

This model uses the top most significantly correlated potential predictors to the response variable, and then performs a _Multicollinearity_ analysis on these to weed out any predictors that cause alarming variance inflation. 

In order to extract the initial model, I extract the top predictors most correlated with the response:


After performing a nested VIF analysis by inserting each predictor if VIF allows it, I get the suggested equation: 



## MODEL 4: Fit All

This model simply fits all! 
...
cat('Model 2 \n')
knitr::kable(vif(model2))

cat('Model 3 \n')
knitr::kable(vif(model3))
```  

We see Models 1 and 3 exhibit _multicollinearity_ between their predictors, whereas Model 2's predictors are fairly independent. We remove predictors with the highest VIF in models 1 and 3, and re-fit each time until we achieve a VIF measure for each predictor of at most 5 (Max SD inflation tolerated is at most 2). We thus arrive at the following models that are fairly free of _multicollinearity_:  

Model 1: mpg ~ wt + hp + am
Model 2 (unchanged): mpg ~ wt + qsec + am
Model 3: mpg ~ drat + qsec + vs + am + gear + carb

### Removing Superfluous Predictors (ANOVA Test)

Model 3 still has too many predictors for my liking. I perform an ANOVA test (starting from a base model mpg ~ drat+am) to remove any predictors that don't inflict a significant change in model variance. 

```{r}
temp <- lm(mpg~drat+am, data=mtcars)
temp1 <- update(temp, mpg~drat+qsec+am)
temp2 <- update(temp, mpg~drat+qsec+vs+am)
temp3 <- update(temp, mpg~drat+qsec+vs+gear+am)
temp4 <- update(temp, mpg~drat+qsec+vs+gear+carb+am)
anova(temp, temp1, temp2, temp3, temp4)

rm(list=c('temp','temp1','temp2','temp3','temp4'))
```
...
model3 <- lm(mpg~drat+qsec+carb+am, data=mtcars)
```
...
textplot("Model 1 (Adjusted Rsquared)", cex=1)
palette(c('cyan','darkgreen','green'))
plot(model1, pch=21, col='black', bg=mtcars$am)

textplot("Model 2 (Stepwise Reg.)", cex=1)
palette(c('cyan','darkgreen','green'))
plot(model2, pch=21, col='black', bg=mtcars$am)

textplot("Model 3 (All Predictors)", cex=1)
palette(c('cyan','darkgreen','green'))
plot(model3, pch=21, col='black', bg=mtcars$am)
```
...
influencePlot(model1, id.col='blue')
influencePlot(model2, id.col='darkgreen')
influencePlot(model3, id.col='brown')
```
